            ...
            async function fetchStats() {
                ...
                try {
                    ...
                    const { data: recentSessions } = await supabase
                        ...
                        .select('duration_seconds, created_at, words_read')
                        .eq('user_id', userId)
                    ...
                    if (recentSessions) {
                        recentSessions.forEach((session: any) => {
                            ...
                            const words = session.words_read || 0;
                    ...
                    // (words/seconds) * 60 = words/minute
                    const averageSpeedWpm = totalSessionDuration > 0 ? Math.round((totalWordsRead / totalSessionDuration) * 60) : 0;
