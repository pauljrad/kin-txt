...
export function RhythmEmphasisPlayer({ text, isActive = true, onComplete }: RhythmEmphasisPlayerProps) {
    ...
    // 2. Rhythm Engine: Local Heuristics (matches main app)
    ...
    // Initialize rhythm on text change
    useEffect(() => {
        const words = text.split(/\s+/).filter(w => w.length > 0);
        setRhythmSpeeds(generateRhythm(words));
        setCurrentIndex(0);
        setWordsRead(0);
    }, [text, generateRhythm]);
    ...
    // ORP Splitting (The "Target Mode" styling for KiN branding)
