...
export function KineticPlayer({
  ...
  const persistReadingTime = useCallback(
    async (force: boolean = false) => {
      ...
      // Only persist if enough time has elapsed, unless forced. (Min 10s or forced exit)
      ...
      try {
        ...
        // 1. Update the document's total cumulative reading time (Library Total)
        await updateDbReadingTime(documentId, currentTotal);

        // 2. Log this specific incremental session for temporal stats (Week/Month/Year)
        if (delta > 0) {
